...
ReadDirections: ;Reads the directions and converts it all to feet.
	LOADI 0		
	STORE A		;safety set it to 0
	IN DIST2	
	CALL Divi
	LOADI A		;A contains the converted mm to feet
	STORE X1
...
FindMe:
	;Runs the code to determine whether it is in the top half or bottom half.
	LOADI X1
	ADD X2
	SUB TOP
...
TOPAREA:
	...
	STORE sav	;stores the total feet to the right remaining into sav (i.e, 12ft width, right sensor detects 4 feet to the right...8 feet remaining from the left...therefore grid 4)
	LOADI 0
	STORE B		;Store B with 0 for divix call
	LOADI sav	; Reloads the remaining distance to the right into AC for divix call
	CALL DiviX
	LOADI B		;B holds the value of the true coordinate
...
BOTTOMAREA:
	...
	STORE sav	;stores this location from feet
	LOADI 0
	STORE B		;Store B with 0 for divix call
	LOADI sav
	CALL DiviX
...
setY4:	
	LOADI 4
	STORE Y7	;If the bottom sensor - 2 is negative or zero stillit is at y=4
	JUMP OutputLocation7Seg
setY3:
	LOADI 3
	STORE Y7	;If the bottom sensor - 2 is positive then it is at y=3
OutputLocation7Seg:
	LOADI X7	; Will output X location sseg1 and output y location to sseg2
	OUT SSEG1
	LOADI Y7
	OUT SSEG2
...
DiviX	
	;Divides the current by 2, to correct the distance/position to actual x coordinate problem
	LOADI sav
	SUB Two
	STORE sav
	LOADI B
...
Divi:
	...
	LOADI A
	ADD One
	STORE A			; A contains the feet amount
	LOADI divsave	; if this is still positive then it must another foot long. 
	JPOS Divi
	RETURN	
